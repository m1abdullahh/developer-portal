...
test('provisions a project end to end', async ({ page }) => {
  ...
  // An option with no P1 recipe is shown rather than hidden, and says so. That honesty is a
  // deliberate design decision, so it is worth asserting.
  await expect(page.getByRole('radio', { name: /Nuxt/ })).toBeDisabled();
...
test('applies the compatibility matrix as options change', async ({ page }) => {
  ...
  // tRPC has no Go equivalent — contradiction 3 from the PRD, stated rather than hidden.
