 ...
 *
 * Module-level rather than per-caller, which is the opposite of the React implementation's local
 * hook — and deliberately so. React's version stays local because the same component set ships to
 * four different state libraries and a module-level store would decide that choice for the project.
 * Nuxt has no such ambiguity: `app/composables/` is auto-imported and a single shared ref is the
 * idiomatic answer, so a toast can be fired from anywhere without threading a provider through.
 */
